    ...
    def build_intersight_api_dict(self, api_results, kwargs):
        ...
        if not kwargs.build_skip == True and api_results.get('Results'):
            def check_for_dotmap_key(kwargs):
                ...
                if isinstance(kwargs.org, str) and not kwargs.intersight_api.get(kwargs.org):
            ...
            for i in api_results.Results:
                ...
                if 'ConfiguredBootMode'    in ikeys: pmoids[iname].boot_mode = i.ConfiguredBootMode
                if 'EnforceUefiSecureBoot' in ikeys: pmoids[iname].enable_secure_boot = i.EnforceUefiSecureBoot
                if 'IpV4Config'            in ikeys: pmoids[iname].ipv4_config = i.IpV4Config
                if 'IpV6Config'            in ikeys: pmoids[iname].ipv6_config = i.IpV6Config
                if 'ManagementMode'        in ikeys: pmoids[iname].management_mode = i.ManagementMode
                if 'MgmtIpAddress'         in ikeys: pmoids[iname].management_ip_address = i.MgmtIpAddress
                if 'Model'                 in ikeys:
                    ...
                    if 'ChassisId'        in ikeys: pmoids[iname].id = i.ChassisId
                    if 'SourceObjectType' in ikeys: pmoids[iname].object_type = i.SourceObjectType
                if 'Organization'  in ikeys: pmoids[iname].organization = kwargs.org_names[i.Organization.Moid]
                if 'PolicyBucket'  in ikeys: pmoids[iname].policy_bucket = i.PolicyBucket
                if 'Selectors'     in ikeys: pmoids[iname].selectors = i.Selectors
                if 'SwitchId'      in ikeys: pmoids[iname].switch_id = i.SwitchId
                if 'Tags'          in ikeys: pmoids[iname].tags = i.Tags
                if 'UpgradeStatus' in ikeys: pmoids[iname].upgrade_status = i.UpgradeStatus
                if 'WorkflowInfo'  in ikeys:
                    if type(i.WorkflowInfo) != kwargs.type_none: pmoids[iname].workflow_moid  = i.WorkflowInfo.Moid
                if 'Distributions' in ikeys: pmoids[iname].distributions  = [e.Moid for e in i.Distributions]
                if 'Source'   in ikeys and 'LocationLink' in ikeys: pmoids[iname].url = i.Source.LocationLink
                if 'Vendor'   in ikeys and   type(i.Vendor) != str: pmoids[iname].vendor_moid = i.Vendor.Moid
    ...
    def calls(self, kwargs):
        ...
        def api_calls(kwargs):
            ...
            for i in range(retries):
                try:
                    ...
                    if 'Your token has expired' in str(e) or 'Not Found' in str(e):
                        kwargs.results = False
                        return kwargs
            ...
            if int(debug_level) >= 6:
                ...
                if payload: pcolor.Cyan('PAYLOAD:'); pcolor.Cyan(json.dumps(payload, indent=4))
            ...
            if re.search('(patch|post)', method):
                if api_results.get('Responses'):
                    for e in api_results.Responses:
                        ...
                        notifications.completed_item(self.category, self.type, kwargs)
        ...
        if kwargs.method == 'get':
            def build_api_args(kwargs_keys, kwargs):
                ...
                if not 'api_filter' in kwargs_keys:
                    ...
                    if re.search('organizations|resource_groups', self.type): api_filter = f"Name in ('{names}')"
                        ...
                        if org_moid: api_filter = f"Name in ('{names}') and Organization.Moid eq '{org_moid}'"
                        else: api_filter = f"Name in ('{names}')"
            ...
            if len(kwargs.names) > 100:
                ...
                elif re.search('expand.+PhysicalDisks', api_args) and kwargs.results.Count > 30: rcount = 1001
            ...
            if 'resource_groups' in okeys and len(kwargs.imm_dict.orgs[org].resource_groups) > 0:
                ...
                for o in kwargs.imm_dict.orgs[org].organizations_to_share_with:
                    if not o in list(kwargs.org_moids.keys()):
                        ...
                        else: notifications.error_organization(o)
            ...
            for org in kwargs.orgs:
                if not org in okeys:
                    ...
                    if 'create_organization' in okeys and kwargs.imm_dict.orgs[org].create_organization == True:
                        ...
                        else: notifications.error_organization(org)
        return kwargs
