...
try:
    ...
    import base64, jinja2, json, numpy, os, re, time
except ImportError as e:
    prRed(f'src/intersight/configure.py - !!! ERROR !!!\n{e.__class__.__name__}')
...
class configure(object):
    ...
    def api_get(self, empty=False, names=None, otype=None, kwargs=None):
        ...
        def empty_results(org, names, kwargs):
            ...
            raise ValueError(f"Empty API query results for {kwargs.uri} (organization={org}, names={', '.join(names)})")
        ...
        for org in orgs:
            if org not in kwargs.org_moids:
                ...
                missing_orgs = True
        if missing_orgs:
            kwargs = api(category=self.category, type='organizations').organizations(kwargs)
        for org in orgs:
    ...
    def cert_file_check(self, expected_type, file_path, item):
        ...
        if not isinstance(file_path, str) or len(file_path.strip()) == 0:
            ...
            raise ValueError(f'`{expected_type}` file path is empty for certificate policy `{item.name}`')
        if not os.path.isfile(file_path):
            pcolor.Red(f'!!! ERROR !!! `{expected_type}` file `{file_path}` was not found for certificate policy `{item.name}`.')
            raise FileNotFoundError(f'`{expected_type}` file `{file_path}` was not found for certificate policy `{item.name}`')
        try:
            ...
            raise OSError(f'Failed to read `{expected_type}` file `{file_path}` for certificate policy `{item.name}`') from exc
        ...
        if 'BEGIN CERTIFICATE' in cert_content:
            try:
                ...
                raise ValueError(f'`{file_path}` is not a valid PEM certificate') from exc
            ...
            try:
                ...
                raise ValueError(f'`{file_path}` is not a valid PEM private key') from exc
            detected_type = 'private_key'
        else:
            pcolor.Red(f'!!! ERROR !!! `{file_path}` is not a valid PEM certificate or private key file.')
            raise ValueError(f'`{file_path}` is not a valid PEM certificate or private key file')
        if detected_type != expected_type:
            pcolor.Red(f'!!! ERROR !!! `{file_path}` contains a `{detected_type}` but `{expected_type}` was expected.')
            raise ValueError(f'`{file_path}` contains `{detected_type}` but `{expected_type}` was expected')
    ...
    def children_check_parent(self, child_type, kwargs):
        ...
        ptitle    = notifications.mod_pol_description((self.type.replace('_', ' ').title()))
    ...
    def children_compare_api_body(self, api_body, kwargs):
        ...
        child_title  = notifications.mod_pol_description(child_type.replace('_', ' ').title())
        ...
        policy_title = notifications.mod_pol_description(((self.type.replace('_', ' ').replace('.', ' : '))).capitalize())
        ...
        parent_title = notifications.mod_pol_description(parent_type.replace('_', ' ').title())
        ...
        for e in kwargs.resources:
            ...
            if child_type in ekeys:
                ...
                for item in e[child_type]:
                    ...
                    child_np = ''
                    child_ns = ''
                    api_body = configure(category=self.category, type=f'{self.type}.{child_type}').create_api_body(item, child_np, child_ns, kwargs)
                    kwargs   = configure(category=self.category, type=f'{self.type}.{child_type}').children_compare_api_body(api_body, kwargs)
    ...
    def configure(self, kwargs):
        ...
        #=====================================================================
        ptitle = notifications.mod_pol_description((self.type.replace('_', ' ').title()))
        notifications.section_begin_org(kwargs.org, ptitle, self.category.title())
        pcolor.LightGray('')
        ...
        if   self.type == 'port':   reconcile_resources = list({v.names[0]:v for v in rdict}.values())
            ...
            notifications.section_end_org(kwargs.org, ptitle, self.category.title())
            return kwargs
            ...
            if self.category == 'profiles' and self.type == 'server':
                reconcile_resources = [e for e in reconcile_resources if e.get('server_family') != 'UCSC885A']
            ...
            if not re.search('id_mapping', self.type):
                for e in list(kwargs.cp.keys()):
                    if len(kwargs.cp[e].names) > 0:
                        ...
                        kwargs = configure(category=category, type=e).api_get(False, names, e, kwargs)
    ...
    def create_api_body(self, item, np, ns, kwargs):
        ...
        try:
            ...
            policy_title = notifications.mod_pol_description((self.type.replace('_', ' ')).capitalize())
            pcolor.Red(
        ...
        if type(api_body) != dict:
            ...
            raise ValueError(f'{policy_title} template did not render to a dictionary payload')
        return api_body
    ...
    def create_bulk_request(self, kwargs):
        def post_to_api(kwargs):
            ...
            kwargs = api(category=self.category, type='bulk_request').calls(kwargs)
            return kwargs
    ...
    def existing_check(self, item, kwargs):
        ...
        def resource_list(k, rname, rtype, item, kwargs):
            ...
            org, new_rname = configure(category=r, type=rtype).determine_resource_organization(add_prefix_suffix=True, resource=rname, kwargs=kwargs)
    ...
    def policies_bios(self, item, kwargs):
        ...
        if 'bios_template' in ikeys:
            ...
            if template_name not in templates:
                ...
                raise ValueError(f'BIOS template "{template_name}" was not found in "intersight.{self.category}.{self.type}.templates"')
    ...
    def policies_ethernet_fc_adapter(self, item, kwargs):
        ...
        if 'adapter_template' in ikeys:
            ...
            if template_name not in templates:
                ...
                pcolor.Red(f'Available templates are: {", ".join(sorted(list(templates.keys())))}')
                raise ValueError(f'{ptitle} template "{template_name}" was not found in "intersight.{self.category}.{self.type}.templates"')
    ...
    def policies_system_qos(self, item, kwargs):
        ...
        for x in range(0, len(item.classes)):
            ...
            elif item.jumbo_mtu: item.classes[x].mtu = 9216
    ...
    def policies_port_children(self, kwargs):
        ...
        for e in port_types:
            if len(port[e].names) > 0:
                ...
                kwargs = configure(category=self.category, type=f'{self.type}.{e}').api_get(True, port[e].names, f'port.{e}', kwargs)
        #=====================================================================
        ...
        def create_and_commit_api(pitem, e, kwargs):
            ...
            if pitem is None:
                return kwargs
            api_body = configure(category=self.category, type=f'port.{e}').create_api_body(pitem, np, ns, kwargs)
            kwargs   = configure(category=self.category, type=f'port.{e}').children_compare_api_body(api_body, kwargs)
            return kwargs
                    ...
                    if trap.get('port'):
                        ...
                        if trap_port in reserved_ports:
                            ...
                            raise ValueError(
                                ...
                                f"Reserved ports: {', '.join(map(str, reserved_ports))}"
                            )
        return kwargs
        ...
        for e in kwargs.resources:
            ...
            if child_type in ekeys:
                for item in e[child_type]:
                    ...
                    reserved_list = shared_functions.vlan_list_full('4043-4047, 4094, 4095')
                    for x in vlans:
                        ...
                        api_body = configure(category=self.category, type=f'{self.type}.{child_type}').create_api_body(item, np, ns, kwargs)
                        kwargs   = configure(category=self.category, type=f'{self.type}.{child_type}').children_compare_api_body(api_body, kwargs)
        if len(kwargs.bulk_list) > 0:
            ...
            if 'switch' in self.type:
                for switch_id in ['A', 'B']:
                    ...
                    if not kwargs.bulk_templates[org].get(sw_template):
                        ...
                        if tdata is None:
                            ...
                            raise ValueError(f'Template "{sw_template}" not found in Intersight Org "{org}" during bulk merge for profile "{name}"')
                ...
                if not kwargs.bulk_templates[org].get(tname):
                    ...
                    if tdata is None:
                        ...
                        raise ValueError(f'Template "{tname}" not found in Intersight Org "{org}" during bulk merge for profile "{name}"')
    ...
    def profiles_chassis_server(self, kwargs):
        ...
        for item in kwargs.resources:
            ikeys    = list(item.keys())
            if item.get('attach_template') is False and any(template_regex.match(k) for k in ikeys):
        ...
        for item in kwargs.resources:
            ...
            if item.get('attach_template') is False and any(template_regex.match(k) for k in ikeys):
                ...
                if 'description' in ikeys:
                    ...
                    api_body['Description'] = f'{name} {self.type.capitalize()} Profile.'
                kwargs.bulk_list.append(api_body)
        if len(kwargs.bulk_list) > 0:
            pcolor.Cyan('')
                ...
                for e in kwargs.cluster_update[k].names:
                    ...
                    while not deploy_complete:
                        ...
                        if kwargs.results.ConfigContext.ControlAction == 'No-op':
                            ...
                            raise TimeoutError(f'Switch profile deployment timed out for "{e}" after {attempts} checks')
                        else:
                            pcolor.Cyan(f'{" "*6}* Deploy Still Occuring on {e}.  Waiting 120 seconds.')
                            time.sleep(120)
        if pending_changes:
            pcolor.LightPurple(f'\n{"-"*108}\n')
        return kwargs
    ...
    def profiles_server_identity_reservations(self, kwargs):
        ...
        for k,v in bulk_list.items():
            if v:
                ...
                kwargs           = configure(category='pools', type=self.type).create_bulk_request(kwargs)
        #=====================================================================
        # Send End Notification and return kwargs
        #=====================================================================
    ...
    def profiles_template_lookup(self, template_type, kwargs):
        ...
        if not final_check: raise ValueError(f'{ptitle} template validation failed for one or more organizations')
        return kwargs
    ...
    def profiles_template_merge(self, item, template_name, ptitle, kwargs):
        ...
        if name in templates:
            ...
            raise ValueError(f'{ptitle} template "{template_name}" was not found')
        ...
        for e in resources:
            ...
            if self.category == 'profiles':
                if 'serial_numbers' in ekeys:
                    ...
                    if remove_count > 0:
                        kwargs.resources[kwargs.resources.index(e)].serial_numbers = []
                ...
                if self.category == 'profiles' and e.get(template_type, None) is not None and e.get('attach_template', True) is False:
                    template_check = True
            names.append(f'{np}{e.name}{ns}')
        if template_check:
            kwargs = self.profiles_template_lookup(template_type, kwargs)
        if len(serials) > 0:
            kwargs = system(category='system', type=self.type).api_get(True, serials, kwargs)
        kwargs.org = org
        kwargs     = self.api_get(True, names, self.type, kwargs)
