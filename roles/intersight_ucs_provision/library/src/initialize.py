...
class begin(object):
    ...
    def functions_to_run(self, kwargs):
        if not kwargs.get('imm_dict') or not kwargs.imm_dict.get('orgs'):
            raise ValueError('No organizations were found in the loaded configuration (missing `imm_dict.orgs`)')
        kwargs.orgs = list(kwargs.imm_dict.orgs.keys())
        if len(kwargs.orgs) > 0:
            kwargs = api(category='system', type='organizations').all_organizations(kwargs)
        else:
            pcolor.Yellow(f'{"-"*108}')
            pcolor.Yellow(f'   No Organizations found in the YAML configuration files.')
            pcolor.Yellow(f'   Confirm that you have created the YAML configuration files correctly and that they contain at least one organization with configurations.')
            pcolor.Yellow(f'{"-"*108}')
            raise ValueError('No organizations found in YAML configuration files')
        #=====================================================================
        ...
        kwargs.system_list    = []
        category_regex = re.compile(r'intersight\.(policies|pools|profiles|system|templates)\.', re.IGNORECASE)
        ...
        if 'system' in kkeys:
            for ptype in kwargs.system_list:
                ...
                if ptype in kwargs.imm_dict.get('system', {}):
                    kwargs = system(category='system', type=f'{ptype}').system(kwargs)
                ...
                for org in kwargs.orgs:
                    ...
                    if ptype in kwargs.imm_dict.orgs[org].get(category, {}):
                        if category == 'profiles' and ptype == 'server':
                            for item in kwargs.imm_dict.orgs[org].get(category, {}).get(ptype, []):
                                if item.get('server_family') == 'UCSC885A':
                                    kwargs.resources = item
                                    kwargs = bmc.build(category='server', type=item.get('server_family')).configure_server(kwargs)
                        kwargs = configure(category=category, type=ptype).configure(kwargs)
        #=====================================================================
        # return kwargs
        #=====================================================================
        return kwargs
