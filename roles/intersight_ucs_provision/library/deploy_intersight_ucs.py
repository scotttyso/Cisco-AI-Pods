...
def cli_arguments() -> Any:
    ...
    parser = argparse.ArgumentParser(description ='Deploy Intersight UCS Module')
    parser = base_arguments(parser)
    parser.add_argument('-c', '--check', action='store_true', help='Boolean flag to enable check mode')
...
def main() -> int:
    ...
    kwargs = initialize.begin('intersight', 'deployment').functions_to_run(kwargs)
